<template>
                    ...
                    <div class="row mb-2 g-3">
                        ...
                        <div class="col-md-auto ms-auto">
                            <div class="d-flex hastck gap-2 flex-wrap">
                                <a v-if="can('roles.store')" @click="create()" data-bs-toggle="modal"
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" :id="'accordionFill' + (role.name).replaceAll(' ', '-')">
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                            :data-bs-target="'#accor_fill' + (role.name).replaceAll(' ', '-')"
                                            aria-expanded="false"
                                            :aria-controls="'accor_fill' + (role.name).replaceAll(' ', '-')">
                                            {{ role.name }}
                                        </button>
                                    </h2>
                                    <div :id="'accor_fill' + (role.name).replaceAll(' ', '-')"
                                        ...
                                        <div class="accordion-body">
                                            <div class="row gy-4">
                                                <div class="col-xl-4 col-md-6"
                                                    v-for="(permission) in myFilter(role.permissions)" :key="permission.id">
                                                    <blockquote
                                                        class="mt-4 blockquote custom-blockquote blockquote-primary rounded mb-0">
                                                        <p class="text-dark mb-2"> {{ permission }}</p>
                                                        <template v-for="name in myFilter2(role.permissions, permission)"
                                                            :key="name.id">
                                            ...
                                            <br>
                                            <div class="d-flex hastck gap-2 flex-wrap">
                                                <a v-if="can('roles.edit')" href="#" class="btn btn-success btn-md col-2"
                                                    data-bs-toggle="modal" data-bs-target="#modalAdd"
                                                    @click.prevent="edit(role.id)">
                                                    <i class="ri-edit-fill align-bottom me-1"></i>Editar
                                                </a>
                                                <button v-if="can('roles.destroy')"  class="btn btn-danger btn-md col-2"
                                                    :disabled="!role.is_modifiable"
                                                    @click.prevent="destroy({ id: role.id, index: index, array: roles })">
                                                    <i class="ri-delete-bin-5-fill align-bottom me-1"></i>Eliminar
                                                </button>
                                            </div>
                                        </div>
                                    </div>
    ...
    <roles-add-edit :form="form" :method_create="method_create" :permissions="permissions" :title_modal="title_modal"
        :submitForm="submitForm" :v="v$">
    </roles-add-edit>
</template>

<script>
import RolesAddEdit from "@/Components/Roles/RolesAddEdit.vue";
import useRoleForm from '@/Composables/Roles/useRoleForm.js';
